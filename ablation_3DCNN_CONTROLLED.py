...
DATA_FOLDER = r"D:\Manisha Phd\dataverse_files"
OUTPUT_FOLDER = r"D:\Manisha Phd\3rd Major revision-Paper#2\Output_3DCNN"
...
# ============================================================
# 3D-CNN SPECTROGRAM CACHE
# NOTE: scipy.signal.spectrogram defaults are preserved from the supplied script.
# ============================================================
...
def get_or_build_spectrograms(nperseg):
    nperseg = int(nperseg)
    noverlap = nperseg // 2
    key = (nperseg, noverlap)
    if key in _spectrogram_cache:
        return _spectrogram_cache[key]

    log(f"Building spectrograms for nperseg={nperseg}, noverlap={noverlap} ...")
    t0 = time.time()
    f_test, t_test, _ = scipy_spectrogram(
        X_1[0, 0, :], fs=SFREQ, nperseg=nperseg, noverlap=noverlap
    )
    freq_mask = f_test <= MAX_FREQ
    n_freqs = int(freq_mask.sum())
    n_times = int(len(t_test))
    freq_values = f_test[freq_mask]

    spec_list = []
    for i in range(len(X_1)):
        channel_specs = []
        for ch_idx in range(X_1.shape[1]):
            f, t, Sxx = scipy_spectrogram(
                X_1[i, ch_idx], fs=SFREQ, nperseg=nperseg, noverlap=noverlap
            )
            channel_specs.append(Sxx[freq_mask, :])
        spec_3d = np.stack(channel_specs, axis=0)
        spec_list.append(np.expand_dims(spec_3d, axis=-1).astype(np.float32))
        if (i + 1) % 5000 == 0:
            log(f"  ... {i+1}/{len(X_1)} epochs converted")

    X_img = np.stack(spec_list, axis=0)
    _spectrogram_cache[key] = X_img
    _spectrogram_meta[key] = {
        "nperseg": nperseg,
        "noverlap": noverlap,
        "n_freq_bins": n_freqs,
        "n_time_bins": n_times,
        "frequency_values_hz": [float(x) for x in freq_values],
        "spectrogram_window": "scipy.signal.spectrogram default (preserved from supplied script)",
    }
    atomic_write_json(OUTPUT_PATH / f"STFT_metadata_nperseg{nperseg}.json", _spectrogram_meta[key])
    log(f"Spectrograms built in {(time.time()-t0)/60:.1f} min. Shape={X_img.shape}")
    return X_img

# ============================================================
# MODEL DEFINITION: 3D-CNN
# IMPORTANT: architecture is preserved from the supplied ablation_3DCNN script.
# ============================================================
def build_3d_cnn_model(input_shape):
    inputs = layers.Input(shape=input_shape)

    def inception_block(x, filters=16, l2_reg=1e-4):
        b1 = layers.Conv3D(filters, (3, 1, 1), padding="same", activation="relu",
                           kernel_regularizer=regularizers.l2(l2_reg))(x)
        b2 = layers.Conv3D(filters, (1, 3, 1), padding="same", activation="relu",
                           kernel_regularizer=regularizers.l2(l2_reg))(x)
        b3 = layers.Conv3D(filters, (1, 1, 3), padding="same", activation="relu",
                           kernel_regularizer=regularizers.l2(l2_reg))(x)
        merged = layers.Concatenate(axis=-1)([b1, b2, b3])
        out = layers.Conv3D(32, (1, 1, 1), padding="same", activation="relu",
...
def build_model_for_config(input_shape, config_params):
    return build_3d_cnn_model(input_shape)
...
def save_3d_grid_heatmap(protocol, summaries, metric_key, fname, title):
    lookup = {(float(s["lr"]), int(s["nperseg"])): finite_or_nan(s.get(metric_key)) for s in summaries if s["protocol"] == protocol}
    mat = np.full((len(NPERSEG_GRID), len(LR_GRID)), np.nan)
    for i, nps in enumerate(NPERSEG_GRID):
        for j, lr in enumerate(LR_GRID):
            mat[i, j] = lookup.get((float(lr), int(nps)), np.nan)
    fig, ax = plt.subplots(figsize=(7, 5))
    im = ax.imshow(mat, aspect="auto")
    ax.set_xticks(range(len(LR_GRID))); ax.set_xticklabels([str(x) for x in LR_GRID])
    ax.set_yticks(range(len(NPERSEG_GRID))); ax.set_yticklabels([str(x) for x in NPERSEG_GRID])
    ax.set_xlabel("Learning rate"); ax.set_ylabel("STFT nperseg")
    ax.set_title(title)
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            v = mat[i, j]
            ax.text(j, i, "nan" if not np.isfinite(v) else f"{v:.3f}", ha="center", va="center")
...
def save_global_investigation_plots(all_summaries):
    for protocol in PROTOCOLS_TO_RUN:
        ...
        for metric_key, short in [
            ...
            save_3d_grid_heatmap(
                protocol, prot, metric_key,
                f"3DCNN_{protocol}_{short}_LRxSTFT_heatmap.png",
                f"3D-CNN {protocol}: {short.replace('_', ' ')} across LR × STFT",
            )
        tags = [s["config_tag"] for s in prot]
        save_subject_difficulty_heatmap(protocol, tags, "3D-CNN")
...
def main():
    ...
    write_manifest({
        ...
        "nperseg_grid": NPERSEG_GRID,
        "protocols": PROTOCOLS_TO_RUN,
        "planned_fold_trainings": int(len(LR_GRID) * len(NPERSEG_GRID) * sum(len(STRATK_FOLDS if p == "stratkfold" else LOSO_FOLDS) for p in PROTOCOLS_TO_RUN)),
        "stft_note": "Full 3 x 3 LR × nperseg factorial sensitivity grid. No best setting is selected from test performance inside this script.",
    })

    log("\n" + "#" * 78)
    log("3D-CNN CONTROLLED ABLATION")
    log(f"Base seed={BASE_SEED}; same fold seed schedule across every LR/STFT config.")
    log(DETERMINISM_STATUS)
    log("Full factorial grid: every LR is evaluated at every nperseg.")
    log("#" * 78)

    all_summaries = []
    for protocol in PROTOCOLS_TO_RUN:
        folds = STRATK_FOLDS if protocol == "stratkfold" else LOSO_FOLDS
        for nperseg in NPERSEG_GRID:
            X_data = get_or_build_spectrograms(nperseg)
            meta = _spectrogram_meta[(int(nperseg), int(nperseg)//2)]
            input_shape = (19, int(meta["n_freq_bins"]), int(meta["n_time_bins"]), 1)
            for lr in LR_GRID:
                tag = f"3DCNN_{protocol}_lr{lr}_nperseg{nperseg}"
                config_params = {
                    "lr": float(lr),
                    "nperseg": int(nperseg),
                ...
                summary = run_one_config(protocol, tag, folds, config_params, X_data, input_shape)
    ...
    log("\n3D-CNN CONTROLLED ABLATION COMPLETE.")
